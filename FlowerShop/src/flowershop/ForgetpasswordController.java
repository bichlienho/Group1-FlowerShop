 ...
 ObservableList<Employee> listData = FXCollections.observableArrayList();
  ...
  public void Employeeforget() {
    ...
    if (!isCurrentUserAdmin()) {
        showAlert(Alert.AlertType.ERROR, "Error Message", "You do not have the necessary permissions to perform this action.");
        return;
    }
            ...
            try (ResultSet rs = checkStmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    try (PreparedStatement updateStmt = connect.prepareStatement(updateSql)) {
                        ...
                        if (affectedRows > 0) {
                            showAlert(Alert.AlertType.INFORMATION, "Information Message", "Password reset successfully!");
                        } else {
                            showAlert(Alert.AlertType.WARNING, "Warning Message", "Password reset unsuccessful.");
                        }
                    }
                } else {
...
private boolean isCurrentUserAdmin() {
    // This method should implement the logic to check if the current user has an Admin role.
    // For example, it could query the database or check a session variable.
